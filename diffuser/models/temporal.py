...
class TemporalUnet_mode_returns_conditioned(nn.Module): # continum task... 
    """
    Task inference specific temporal u net, with no conditioning.
    """ 
    #TODO use cross attention... 
    ...
    def forward(self, x, time,mode,returns=None,use_dropout=True,force_dropout=False):
        '''
            x : [ batch x horizon x transition ] transition=S+A+TASK+RTG
            time : [batch x 1]
            mode : [batch x 1]
        ...
        if self.returns_condition:
            ...
            t = torch.cat([t, returns_embed], dim=-1) # TODO test this
